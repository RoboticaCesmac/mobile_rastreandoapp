...
export default function SeusExamesDeRastreioColoDeUteroFeminino() {
    ...
    const fetchExames = async () => {
      if (user) {
        ...
        if (userDoc.exists()) {
          ...
          setProximoExame(userData.proximoExameColoDeUtero);
  ...
  const handleConfirm = (date: Date) => {
    ...
    if (user) {
      ...
      updateDoc(userDocRef, {
        proximoExameColoDeUtero: date.toISOString(),
        examesAnterioresColoDeUtero: updatedExames
      });
    }
    hideDatePicker();
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Seus Exames de Rastreio - Colo de Útero</Text>
